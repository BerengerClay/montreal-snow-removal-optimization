...
def generate_city_graph(grid_size, edge_prob=0.5, extra_edges=5, seed=43):
    np.random.seed(seed)
    G = nx.Graph()
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            if j < grid_size - 1 and np.random.rand() < edge_prob:
                G.add_edge((i, j), (i, j + 1), weight=np.linalg.norm(np.array((i,j)) - np.array((i+1,j))))
...
def main():
    ...
    plot_graph_and_path(G, cpp_path, 'Chinese Postman Problem')
