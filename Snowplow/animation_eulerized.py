...
def find_odd_degree_nodes(G):
    return [node for node, degree in G.degree() if degree % 2 != 0]
...
def chinese_postman_path(G):
    """Find the Chinese Postman Path for an Eulerian graph."""
    return list(nx.eulerian_circuit(G))

def create_animation(G, path, filename):
    ...
    for i in range(len(path) + 1):
        plt.clf()
        nx.draw(G, pos, with_labels=False, node_size=10, edge_color='lightgray')
        edges = path[:i]
        nx.draw_networkx_edges(G, pos, edgelist=edges, edge_color='blue', width=1.5, arrows=True, arrowstyle='-|>', arrowsize=10)
        plt.title("Chinese Postman Path")
