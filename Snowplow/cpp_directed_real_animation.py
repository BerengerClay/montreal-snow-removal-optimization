...
def animate_postman_path(G, G_eulerian, path, added_edges, converted_edges, filename):
    pos = {node: (node[1], node[0]) for node in G.nodes()}
...
def main():
    ...
    fig, ax = plt.subplots(figsize=(8, 8))
    plot_graph(G_scc, 'Largest Strongly Connected Component', node_size=1, ax=ax)
    plt.show()

    G_eulerian, added_edges, converted_edges = make_eulerian(G_scc)
    if nx.is_eulerian(G_eulerian):
        print("Graph is now Eulerian: True")
        path = chinese_postman_path(G_eulerian)
        animate_postman_path(G_scc, G_eulerian, path, added_edges, converted_edges, 'outremont_postman_animation.mp4')
    else:
